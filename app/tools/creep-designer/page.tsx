    ...
    <div className="min-h-screen screeps-bg">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-12">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={resetAll}
            className="px-4 py-2 btn-secondary rounded-lg text-sm font-medium text-gray-300"
          >
            重置
          </button>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="grid sm:grid-cols-2 gap-3">
              {parts.map((part, index) => (
                <div key={part.type} className="bg-gray-800/60 backdrop-blur-sm rounded-xl p-4 border border-indigo-500/10 card-hover">
                  <div className="flex items-center justify-between mb-3">
                    <span className="font-medium capitalize text-sm text-gray-200">{part.type.replace('_', ' ')}</span>
                    <span className="text-xs text-gray-500">{BODY_PART_COSTS[part.type]} 能量</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => updatePartCount(index, -1)}
                      className="w-8 h-8 bg-gray-700/80 hover:bg-gray-600 text-white rounded-lg transition-colors flex items-center justify-center font-bold border border-gray-600/50"
                    >
                      -
                    </button>
                    <input
                      ...
                      className="w-14 h-8 px-2 bg-gray-700/80 border border-gray-600/50 rounded-lg text-white text-center focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
                    />
                    <button
                      onClick={() => updatePartCount(index, 1)}
                      className="w-8 h-8 bg-gray-700/80 hover:bg-gray-600 text-white rounded-lg transition-colors flex items-center justify-center font-bold border border-gray-600/50"
            ...
            {totalParts > 0 && (
              <div className="bg-gray-800/60 backdrop-blur-sm rounded-xl p-4 border border-indigo-500/10">
                <div className="text-xs text-gray-500 mb-3">部件预览</div>
                  ...
                  {parts.flatMap(part => 
                    Array(part.count).fill(null).map((_, i) => (
                      <div
                        ...
                        } ${part.boost ? 'ring-2 ring-indigo-400/50' : ''}`}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-800/60 backdrop-blur-sm rounded-xl p-4 border border-indigo-500/10">
              <div className="grid grid-cols-4 gap-3">
                <div className="col-span-1">
                  <label className="text-xs text-gray-400 mb-1.5 block">Tick(秒)</label>
                  <input
                    type="text"
                    inputMode="numeric"
                    pattern="[0-9]*"
                    ...
                    className="w-full h-9 px-2 bg-gray-700/80 border border-gray-600/50 rounded-lg text-white text-center focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
                  />
                </div>
                <div className="col-span-3">
                  <label className="text-xs text-gray-400 mb-1.5 block">控制器等级</label>
                  <select
                    value={controllerLevel}
                    onChange={(e) => setControllerLevel(parseInt(e.target.value))}
                    className="w-full h-9 px-2 bg-gray-700/80 border border-gray-600/50 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
                  >
            ...
            <div className="bg-gray-800/60 backdrop-blur-sm rounded-xl p-4 border border-indigo-500/10">
              <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-gray-300">孵化成本</span>
                  <span className={`text-lg font-bold ${isOverBudget ? 'text-red-400' : 'text-indigo-400'}`}>{stats.totalCost}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-gray-300">部件数</span>
                  <span className={`text-lg font-bold ${isOverLimit ? 'text-red-400' : 'text-green-400'}`}>{totalParts}/50</span>
                </div>
                <div className="flex justify-between items-center">
            ...
            {/* Remaining Energy */}
            <div className={`rounded-xl p-4 border ${isOverBudget ? 'bg-red-900/20 border-red-500/30' : 'bg-gray-800/60 border-indigo-500/10'}`}>
              <div className="flex justify-between items-center">
                <span className="text-xs text-gray-300">剩余能量 (Lv.{controllerLevel})</span>
                <span className={`text-xl font-bold ${isOverBudget ? 'text-red-400' : 'text-green-400'}`}>{remainingEnergy}</span>
              </div>
            </div>
            ...
            <div className="bg-gray-800/60 backdrop-blur-sm rounded-xl p-4 border border-indigo-500/10">
              ...
              <div className="bg-gray-900/80 rounded-lg p-3 text-xs font-mono break-all mb-3 min-h-[2rem] border border-gray-700/50">
                {bodyProfile || '{}'}
              </div>
              ...
              <div className="mt-3 pt-3 border-t border-gray-700/50">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 h-9 px-3 bg-gray-700/80 border border-gray-600/50 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
                  />
                  <button onClick={importProfile} className="px-4 py-2 btn-secondary rounded-lg text-sm font-medium text-gray-300">
                    导入
                  </button>
                </div>
        ...
        <div className="mt-8 pt-8 border-t border-indigo-500/10">
          <h2 className="text-lg font-semibold mb-4 text-gray-200">时间维度统计</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { title: '每 Tick', data: timeStats.perTick, color: 'text-indigo-400' },
              { title: '每 Work', data: timeStats.perUnit, color: 'text-green-400' },
              { title: '每小时', data: timeStats.perHour, color: 'text-yellow-400' },
              { title: '每天', data: timeStats.perDay, color: 'text-purple-400' },
            ].map(({ title, data, color }) => (
              <div key={title} className="bg-gray-800/60 backdrop-blur-sm rounded-xl p-4 border border-indigo-500/10">
